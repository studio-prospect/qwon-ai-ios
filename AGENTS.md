...
- run `git diff --check` before commit when editing text files (CI enforces on PRs; optional local hook: `./tools/scripts/install-git-hooks.sh` — skips if another `pre-commit` exists; use `--force` to backup-then-replace)
